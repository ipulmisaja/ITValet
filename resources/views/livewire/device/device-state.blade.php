                            ...
                            @foreach ($states as $state)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <p class="my-1 text-gray-900 dark:text-white">{{ $state->namedevice ?? '-'}}</p>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="flex flex-nowrap place-items-center my-1 text-gray-500 dark:text-gray-400 space-x-2">
                                    ...
                                    <td class="px-6 py-4">
                                        @isset($state->device->maintenance)
                                            @if (count($state->device->maintenance) == 0)
                                                <x-pages.condition condition="baik" />
                                            @else
                                                <x-pages.condition :condition="$state->device->maintenance->sortBy('created_at')->last()->condition" />
                                            @endif
                                        @else
                                            <x-pages.condition condition="baik" />
                                        @endisset
                                    ...
                                    @role('admin|umum')
                                        <td class="px-6 py-4">
                                            <div class="flex place-items-center space-x-3">
                                                {{-- Edit Perangkat --}}
                                                @can('update-device')
                                                    <x-pages.cell-button :route="route('device.edit', $state->device_id)" tooltip="Perangkat"
                                                        color="text-primary-500 hover:text-primary-600" icon="computer-desktop" />
                                                @endcan

                                                {{-- Alokasi Perangkat --}}
                                                @canany(['create-device-state', 'update-device-state'])
                                                    <x-pages.cell-button
                                                        route="{{ !is_null($state->id) ? route('device-state.edit', $state->id) : route('device-state.create', $state->device->id) }}"
                                                        tooltip="Alokasi"
                                                ...
                                                @canany(['create-device-maintenance', 'update-device-maintenance'])
                                                    <x-pages.cell-button :route="route('device-maintenance', $state->device_id)" tooltip="Pemeliharaan"
                                                        color="text-yellow-400 hover:text-yellow-500" icon="shield-exclamation" />
                                                @endcanany

                                                @can('delete-device-state')
                                                    <x-pages.delete-cell-button label="Hapus" :id="$state->id" />
                                                @endcan
                                            </div>
                                        </td>
                                    @endrole
